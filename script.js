...
const ELEC_STARTS = {
  ...
  12: new Date(2025, 10, 1, 0, 0, 0),   // 01/11/2025 00:00
  17: new Date(2025, 9, 23, 13, 0, 0),  // 23/10/2025 13:00
  18: new Date(2025, 10, 1, 0, 0, 0),   // 01/11/2025 00:00
  19: new Date(2025, 10, 1, 0, 0, 0),   // 01/11/2025 00:00
  20: new Date(2025, 10, 1, 0, 0, 0)    // 01/11/2025 00:00
};
...
let selectedMotors = new Set(); // até 2 motores para visualizar no gráfico
...
function postLoad() {
  ...
  // Renderizar datas de resistências
  renderResistanceDates();
...
function toggleMotor(m, on, pillEl) {
  if (on) {
    ...
    // Limitar a 2 motores selecionados para visualização
    if (selectedMotors.size < 2) {
      selectedMotors.add(m);
    } else {
      // Se já tem 2, remover o primeiro e adicionar o novo
      const first = selectedMotors.values().next().value;
      selectedMotors.delete(first);
      selectedMotors.add(m);
    }
  } else {
    activeMotors.delete(m);
    selectedMotors.delete(m);
...
function updateChartInfo() {
  if (selectedMotors.size === 0) {
    chartInfo.textContent = 'Selecione até 2 motores para visualizar';
  } else if (selectedMotors.size === 1) {
    const m = Array.from(selectedMotors)[0];
    chartInfo.textContent = `Visualizando: UG#${String(m).padStart(2, '0')}`;
  } else {
    const motors = Array.from(selectedMotors).sort((a, b) => a - b);
    chartInfo.textContent = `Visualizando: UG#${String(motors[0]).padStart(2, '0')} e UG#${String(motors[1]).padStart(2, '0')}`;
  }
}
...
function buildTraces() {
  ...
  // Mostrar apenas motores selecionados
  for (let m of selectedMotors) {
    ...
    traces.push({
      ...
      line: { color: colorForMotor(m), width: 3 },
...
function renderResistanceDates() {
  resistanceGrid.innerHTML = '';
  for (let m of ELEC_MOTORS) {
    const date = ELEC_STARTS[m];
    const item = document.createElement('div');
    item.className = 'resistance-item';
    item.innerHTML = `
      <div class="resistance-motor">Motor #${String(m).padStart(2, '0')}</div>
      <div class="resistance-date">${date.toLocaleString('pt-BR')}</div>
    `;
    resistanceGrid.appendChild(item);
  }
}

function renderSummaryCard(stats, totalLiters, totalMoney) {
  ...
  for (let m = 1; m <= 23; m++) {
    ...
    const s = stats[m];
    if (!s) continue;
    const ok = (s.lastTemp != null && s.lastTemp > 50);
    if (ok) available++;
    else unavailable++;
  }
  ...
  const items = [
    { label: 'Motores disponíveis', value: available, color: 'success' },
    { label: 'Motores indisponíveis', value: unavailable, color: 'danger' },
    { label: 'Economia total (L)', value: fmt1(totalLiters) },
    { label: 'Economia total (R$)', value: money(totalMoney) }
  ];
...
function renderChartMarkers() {
  ...
  if (selectedMotors.size < 2) return;
  
  const motors = Array.from(selectedMotors).sort((a, b) => a - b);
  const m1 = motors[0], m2 = motors[1];
  
  const series1 = (dataByMotor[m1] || []).slice(startIdx, endIdx + 1);
  const series2 = (dataByMotor[m2] || []).slice(startIdx, endIdx + 1);
  const xs = timestamps.slice(startIdx, endIdx + 1);
  
  // Encontrar pontos acima de 50°C
  let firstAbove50_m1 = -1, lastAbove50_m1 = -1;
  let firstAbove50_m2 = -1, lastAbove50_m2 = -1;
  
  for (let i = 0; i < series1.length; i++) {
    if (series1[i] != null && series1[i] > 50) {
      if (firstAbove50_m1 === -1) firstAbove50_m1 = i;
      lastAbove50_m1 = i;
    }
  }
  
  for (let i = 0; i < series2.length; i++) {
    if (series2[i] != null && series2[i] > 50) {
      if (firstAbove50_m2 === -1) firstAbove50_m2 = i;
      lastAbove50_m2 = i;
    }
  }
  
  // Calcular tempo entre marcadores
  if (firstAbove50_m1 !== -1 && firstAbove50_m2 !== -1) {
    const time1 = xs[firstAbove50_m1];
    const time2 = xs[firstAbove50_m2];
    ...
    item.innerHTML = `
      <span class="marker-dot" style="background:${colorForMotor(m1)}"></span>
      <span>UG#${String(m1).padStart(2, '0')} atingiu 50°C às ${time1.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</span>
    `;
    chartMarkers.appendChild(item);
    
    const item2 = document.createElement('div');
    item2.className = 'marker-item';
    item2.innerHTML = `
      <span class="marker-dot" style="background:${colorForMotor(m2)}"></span>
      <span>UG#${String(m2).padStart(2, '0')} atingiu 50°C às ${time2.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</span>
    `;
    chartMarkers.appendChild(item2);
    
    const item3 = document.createElement('div');
    item3.className = 'marker-item';
    item3.style.background = '#f0f9ff';
    item3.innerHTML = `
      <span style="color: var(--brand); font-weight: 700;">⏱️ Diferença: ${String(diffH).padStart(2, '0')}:${String(diffM).padStart(2, '0')} h</span>
    `;
    chartMarkers.appendChild(item3);
  }
}
...
function renderAll() {
  ...
  renderCards(stats);
  renderSummaryCard(stats, totalLiters, totalMoney);
